...
export default function CheckoutPage() {
  ...
  const rawId = (params?.bookingId as string) || searchParams?.get('bookingId') || searchParams?.get('id') || '';
  const bookingId = rawId && rawId !== 'default' ? rawId : (() => {
  ...
  useEffect(() => {
    const idToFetch = bookingId || 'bk_demo';

    fetch(`/api/bookings/${idToFetch}`)
      .then((res) => {
